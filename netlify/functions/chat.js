...
export default async function handler(req, context) {
  ...
  try {
    ...
    if (intentTags.includes('jailbreak-attempt') && !traceSource) {
      context.waitUntil(sendJailbreakAlert(lastUserMessage))
    }
    ...
    const langInstruction = lang === 'en'
      ? `The user is browsing in English. You MUST respond in English. Contact email: omarmostafapm@gmail.com\ninternal_ref: ${canary}`
      : `El usuario navega en español. Responde en español. Email de contacto: omarmostafapm@gmail.com\ninternal_ref: ${canary}`
    ...
    if (ragEnabled) {
      ...
      if (firstResponse.stop_reason === 'tool_use') {
        ...
        const toolResultContent = ragResult.chunks
          ...
          : 'No relevant content found in portfolio articles. You MUST NOT fabricate project details. Say you don\'t have that information and suggest contacting Omar directly.'
    ...
    if (langfuse) context.waitUntil(langfuse.flushAsync())
      ...
      try {
        ...
        if (precomputedResponse) {
          ...
          if (containsFingerprint(precomputedText) || precomputedText.includes(canary)) {
            ...
            controller.close()
            context.waitUntil(sendJailbreakAlert(`[PROMPT LEAK BLOCKED] User: ${lastUserMessage}`))
        ...
        if (!leakDetected) {
          ...
          if (process.env.ENABLE_ONLINE_SCORING === 'true' && langfuse && trace && fullOutput) {
            context.waitUntil(scoreTrace(trace.id, lastUserMessage, fullOutput, ragUsed, langfuse))
          }
              ...
              if (event.type === 'content_block_delta' && event.delta.type === 'text_delta') {
                ...
                if (fallbackOutput.length % 200 < chunk.length || fallbackOutput.length < 200) {
                  if (containsFingerprint(fallbackOutput) || fallbackOutput.includes(canary)) {
                    ...
                    controller.enqueue(encoder.encode('data: [DONE]\n\n'))
                    controller.close()
                    context.waitUntil(sendJailbreakAlert(`[PROMPT LEAK BLOCKED - FALLBACK] User: ${lastUserMessage}`))
                    if (langfuse) context.waitUntil(langfuse.flushAsync())
                    return
                  }
                }
        ...
        try {
          const errorText = lang === 'en'
            ? 'Sorry, something went wrong. Try again or reach out at omarmostafapm@gmail.com.'
            : 'Lo siento, algo ha fallado. Inténtalo de nuevo o escríbeme a omarmostafapm@gmail.com.'
...
// Zero added latency: runs after response is sent via context.waitUntil()
// ---------------------------------------------------------------------------
...
async function scoreTrace(traceId, userMessage, response, ragUsed, langfuse) {
  try {
    ...
    const scoringResponse = await client.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `Rate this chatbot response (Omar's CV chatbot). Respond ONLY with JSON.
