...
export function useArticleSeo(opts: ArticleSeoOpts) {
  useEffect(() => {
    ...
    upsertMeta('name', 'author', 'Omar Mostafa Mohaseb')
    ...
    for (const { hreflang, href } of [
      ...
      { hreflang: 'x-default', href: `https://omarmostafa.org/${defaultSlug}` },
    ]) {
...
export function useHomeSeo({ lang, title, description }: { lang: string; title: string; description: string }) {
  useEffect(() => {
    ...
    const canonical = lang === 'en' ? 'https://omarmostafa.org/en' : 'https://omarmostafa.org/'
