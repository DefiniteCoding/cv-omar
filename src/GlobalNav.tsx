...
import { Sun, Moon, House, X, ChevronRight, Github } from 'lucide-react'
import { translations, type Lang } from './i18n'
import { getAltPaths, getPageTitles, getSectionLabels, getArSlugs } from './articles/registry'
...
function useLanguageBanner(lang: Lang) {
  ...
  useEffect(() => {
    ...
    const mismatch = (lang === 'ar' && browserPrefersEn) || (lang === 'en' && !browserPrefersEn)
    if (!mismatch) return
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <a
        ...
        className="w-10 h-10 rounded-full bg-card border border-border flex items-center justify-center shadow-lg hover:border-[#0A66C2]/50 hover:shadow-xl transition-colors"
        aria-label="LinkedIn"
      >
        <svg viewBox="0 0 24 24" className="w-4.5 h-4.5 w-[18px] h-[18px] text-[#0A66C2]" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
      </a>
      <Link
        to={altPath}
...
export default function GlobalNav() {
  ...
  const bannerMessage = showBanner ? (
    ...
    >
      ...
      >
        {t.ui.languageBannerSwitchPrefix}{lang === 'ar' ? <FlagEN className="w-3.5 h-3.5 mx-0.5" /> : <FlagAR className="w-3.5 h-3.5 mx-0.5" />}{t.ui.languageBannerSwitchLang}
      </button>
      <button
        onClick={dismiss}
