  ...
  return (
    <ArticleLayout lang={lang}>
      ...
      <Photo2
        ...
        items={[
          { src: '/business-os/hero-storefront.webp', alt: lang === 'ar' ? 'Fachada de Santifer iRepair — tienda de reparación de móviles en Madrid' : 'Santifer iRepair storefront — phone repair shop in Madrid', width: 900, height: 1200 },
          { src: '/business-os/hero-storefront-urban.webp', alt: lang === 'ar' ? 'Santifer iRepair de noche — vista urbana con la tienda iluminada' : 'Santifer iRepair at night — urban view with the shop lit up', width: 800, height: 1067 },
        ]}
        className="mb-8"
      />
      ...
      <article className="prose-custom">
        ...
        <Photo2
          ...
          items={[
            { src: '/business-os/counter-organized-front.webp', alt: lang === 'ar' ? 'Interior de Santifer iRepair — lo que ve el cliente al entrar: mostrador con iMac, logo y lámpara esférica' : 'Santifer iRepair interior — what the customer sees walking in: counter with iMac, logo and spherical lamp', width: 360, height: 480 },
            { src: '/business-os/after-elevator-lamp.webp', alt: lang === 'ar' ? 'Montacargas de madera junto a lámpara esférica y logo Santifer — conexión entre mostrador y taller' : 'Wooden dumbwaiter next to spherical lamp and Santifer logo — connection between counter and workshop', width: 360, height: 480 },
          ]}
          caption={lang === 'ar' ? 'Lo que ve el cliente al entrar / El montacargas que conecta mostrador y taller' : 'What the customer sees walking in / The dumbwaiter connecting counter and workshop'}
        />
        ...
        <CaseStudyCta
          ...
          ctaHref={lang === 'ar' ? '/agente-ia-jacobo' : '/ai-agent-jacobo'}
        />
        ...
        <Photo2
          ...
          items={[
            { src: '/business-os/before-chaos-desktop.webp', alt: lang === 'ar' ? 'Escritorio con Checkout POS, calendario, notas y cajonera de piezas — julio 2015' : 'Desktop with Checkout POS, calendar, notes and parts drawers — July 2015', width: 1200, height: 900 },
            { src: '/business-os/before-multisystem.webp', alt: lang === 'ar' ? 'Checkout POS, Slack, scripts de facturación y recordatorios — todo abierto a la vez' : 'Checkout POS, Slack, invoicing scripts and reminders — all open at once', width: 1200, height: 838 },
          ]}
          caption={lang === 'ar' ? 'POS + calendario + notas (2015) / POS + Slack + scripts + recordatorios (2019)' : 'POS + calendar + notes (2015) / POS + Slack + scripts + reminders (2019)'}
          className="mb-8"
        />
            ...
            <div key={flow.name} className="scroll-mt-20">
              ...
              {idx === 0 && (
                <Photo2 editorId="repair-counter-watch"
                  items={[
                    { src: '/business-os/after-counter-pov.webp', alt: lang === 'ar' ? 'Mostrador de Santifer desde el punto de vista del empleado — iMac con Airtable abierto, citas programadas' : 'Santifer counter from employee POV — iMac with Airtable open, appointments scheduled', width: 800, height: 1067 },
                    { src: '/business-os/after-apple-watch-booking.webp', alt: lang === 'ar' ? 'Apple Watch del técnico mostrando la próxima cita' : 'Technician Apple Watch showing next appointment', width: 800, height: 1067 },
                  ]}
                  caption={lang === 'ar' ? 'Planta baja: citas en Airtable / Planta alta: carga de trabajo en la muñeca' : 'Ground floor: appointments in Airtable / Upstairs: workload on the wrist'}
                />
              )}
              {idx === 1 && (
                <Photo2 editorId="proc-accessories"
                  items={[
                    { src: '/business-os/accessories-cases-shelf.webp', alt: lang === 'ar' ? 'Fundas organizadas por modelo' : 'Cases organized by model', width: 1200, height: 900 },
                    { src: '/business-os/accessories-led-shelf.webp', alt: lang === 'ar' ? 'Mueble LED con accesorios' : 'LED display shelf with accessories', width: 1200, height: 900 },
                  ]}
                  caption={lang === 'ar' ? 'Cada SKU sincronizado con Airtable — precio, margen, rotación' : 'Every SKU synced with Airtable — price, margin, rotation'}
                />
              )}
              {idx === 2 && (
                <Photo1 editorId="content-before-after" src="/business-os/web-before-after.webp" alt={lang === 'ar' ? 'Before/after de reparaciones reales' : 'Real repair before/after'} width={1560} height={1040} caption={lang === 'ar' ? 'Before/after reales + reseñas — fotos del ERP, specs de gsmarena-api' : 'Real before/after + reviews — photos from ERP, specs from gsmarena-api'} />
              )}
              {idx === 3 && (
                <Photo1 editorId="lifecycle-automations" src="/business-os/automatizaciones-mensajes.webp" alt={lang === 'ar' ? 'Automatizaciones de mensajes en Airtable' : 'Airtable messaging automations'} width={1560} height={1040} caption={lang === 'ar' ? 'Automatizaciones de comunicación: reseñas por tier, notificaciones, campañas y WhatsApp' : 'Communication automations: tier-based reviews, notifications, campaigns and WhatsApp'} />
              )}
              ...
              <BulletList editorId={`e2e-${idx}-details`} items={flow.details as readonly string[]} />
                ...
                {idx === 0 && (
                  <>
                    <Photo1
                      ...
                      alt={lang === 'ar' ? 'Automatización Airtable — webhook unificado que decide el flujo según stock' : 'Airtable automation — unified webhook that routes the flow based on stock'}
                      width={1560} height={1040}
                      caption={lang === 'ar' ? 'Webhook unificado — si hay stock crea la OT, si no cancela cita y genera pedido' : 'Unified webhook — creates work order if in stock, cancels appointment and auto-orders if not'}
                    />
                    <Photo2
                      editorId="repair-parts"
                      items={[
                        { src: '/business-os/parts-organized-screens.webp', alt: lang === 'ar' ? 'Pantallas de iPhone organizadas por modelo en estantería' : 'iPhone screens organized by model on shelf', width: 1200, height: 900 },
                        { src: '/business-os/parts-pcb-microscope.webp', alt: lang === 'ar' ? 'Técnico trabajando con microscopio en placa PCB' : 'Technician working with microscope on PCB board', width: 1200, height: 900 },
                      ]}
                      caption={lang === 'ar' ? 'Piezas organizadas por Airtable / El técnico se centra en lo que importa' : 'Parts organized by Airtable / The technician focuses on what matters'}
                    />
                    <Photo1
                      editorId="repair-ot"
                      src="/business-os/airtable-ot-repair.webp"
                      alt={lang === 'ar' ? 'Interfaz Airtable — pestaña de reparación con piezas, garantías y accesorios' : 'Airtable interface — repair tab with parts, warranties and accessories'}
                      width={1560} height={1040}
                      caption={lang === 'ar' ? 'OT en Airtable — reparación, garantías y accesorios' : 'Work order in Airtable — repair, warranties and accessories'}
                    />
                    <Photo1
                      editorId="repair-warranties"
                      src="/business-os/airtable-warranties.webp"
                      alt={lang === 'ar' ? 'Interfaz Airtable de garantías' : 'Airtable warranties interface'}
                      width={1560} height={1040}
                      caption={lang === 'ar' ? 'Gestión de garantías: pieza → proveedor → pedido original → estado de reclamación' : 'Warranty management: part → supplier → original order → claim status'}
                    />
                  </>
                )}
                ...
                {idx === 1 && (
                  <>
                    <Photo1 editorId="proc-inventory" className="mt-4" src="/business-os/airtable-inventory.webp" alt={lang === 'ar' ? 'Interfaz Airtable de inventario — pieza con foto real, stock, ubicación física' : 'Airtable inventory interface — part with real photo, stock, physical location'} width={1560} height={1040} caption={lang === 'ar' ? 'Ficha de pieza: stock, proveedor, foto real y ubicación física (armario, cajón)' : 'Part record: stock, supplier, real photo and physical location (cabinet, drawer)'} />
                    <Photo1 editorId="proc-inventory-2" src="/business-os/airtable-inventory-2.webp" alt={lang === 'ar' ? 'Segunda vista de inventario — foto de ubicación real' : 'Second inventory view — real shelf location photo'} width={1560} height={1040} caption={lang === 'ar' ? 'Foto de ubicación real: el empleado ve la estantería exacta' : 'Real location photo: the employee sees the exact shelf'} />
                    <Photo1 editorId="proc-orders" src="/business-os/airtable-purchase-orders.webp" alt={lang === 'ar' ? 'Interfaz Airtable de pedidos' : 'Airtable purchase orders interface'} width={1560} height={1040} caption={lang === 'ar' ? 'Stock bajo → pedido automático → recepción → inventario actualizado' : 'Low stock → auto order → reception → inventory updated'} />
                  </>
                )}
                ...
                {idx === 2 && (
                  <>
                    <Photo1 editorId="content-catalog" className="mt-4" src="/business-os/airtable-models-catalog.webp" alt={lang === 'ar' ? 'Catálogo de modelos en Airtable' : 'Airtable models catalog'} width={1560} height={1040} caption={lang === 'ar' ? 'Catálogo de modelos: de aquí salen landings, precios y fotos' : 'Models catalog: this feeds landings, prices and photos'} />
                    <Photo1 editorId="content-landing" src="/business-os/web-landing-hero.webp" alt={lang === 'ar' ? 'Landing de reparación generada desde Airtable' : 'Repair landing generated from Airtable'} width={1560} height={1040} caption={lang === 'ar' ? 'Landing 100% generada: precio, reseñas, specs (gsmarena-api) y SEO dinámico' : 'Landing 100% generated: price, reviews, specs (gsmarena-api) and dynamic SEO'} />
                    <Photo1 editorId="content-repairs" src="/business-os/web-repairs-catalog.webp" alt={lang === 'ar' ? 'Catálogo de reparaciones iPhone 11' : 'iPhone 11 repair catalog'} width={1560} height={1040} caption={lang === 'ar' ? 'Catálogo de averías por modelo — generado desde el ERP' : 'Repair catalog by model — generated from the ERP'} />
                  </>
                )}
                      ...
                      <div className="bg-background/50 rounded-lg border border-border p-4">
                        ...
                        <div className="bg-primary/5 border-l-2 border-primary/30 pl-3 py-2">
                          <p className="text-xs text-muted-foreground mb-1 font-medium">
                            {lang === 'ar' ? 'Respuesta de Santifer iRepair:' : 'Santifer iRepair response:'}
                          </p>
                          ...
                          <p className="text-[10px] text-primary/50 mt-1.5 flex items-center gap-1">
                            ...
                            {lang === 'ar' ? 'Contexto del CRM: iPad, cita gestionada vía Jacobo (chatbot WhatsApp), reparación express 1h' : 'CRM context: iPad, appointment via Jacobo (WhatsApp chatbot), 1h express repair'}
                          </p>
                        </div>
                      </div>
                      <div className="bg-background/50 rounded-lg border border-border p-4">
                        ...
                        <div className="bg-primary/5 border-l-2 border-primary/30 pl-3 py-2">
                          ...
                          <p className="text-[10px] text-primary/50 mt-1.5 flex items-center gap-1">
                            {AIRTABLE_SVG('w-3 h-3 text-[#18BFFF]')}
                            {lang === 'ar' ? 'Contexto del CRM: alerta reseña negativa, cristal iPhone, ticket reabierto, resolución mismo día' : 'CRM context: negative review alert, iPhone glass, ticket reopened, same-day resolution'}
                          </p>
                        </div>
                      </div>
                    </div>
        ...
        <CaseStudyCta
          ...
          ctaHref={lang === 'ar' ? 'mailto:hola@omarmostafa.org?subject=Programmatic SEO' : 'mailto:hi@omarmostafa.org?subject=Programmatic SEO'}
        />
          ...
          {t.sections.crossCutting.items.map((cap, idx) => {
            ...
            <div key={cap.name} className="scroll-mt-20">
              ...
              <BulletList editorId={`cc-${idx}-details`} items={cap.details as readonly string[]} />
                ...
                {/* CC0: Data Guardrails */}
                {idx === 0 && (
                  <Photo1 editorId="cc-guardrails" className="mt-4" src="/business-os/airtable-ot-terminal.webp" alt={lang === 'ar' ? 'Interfaz Airtable — orden de trabajo con datos del terminal' : 'Airtable interface — work order with device data'} width={1560} height={1040} caption={lang === 'ar' ? 'Formulario de entrada — cada campo validado automáticamente' : 'Intake form — every field automatically validated'} />
                )}
                ...
                {idx === 3 && (
                  <>
                    <Photo2
                      ...
                      className="mt-4"
                      items={[
                        { src: '/business-os/digital-signage-storefront.webp', alt: lang === 'ar' ? 'Pantalla digital en escaparate — reparación de móviles' : 'Digital signage in storefront — mobile repair', width: 600, height: 800 },
                        { src: '/business-os/digital-signage-storefront-2.webp', alt: lang === 'ar' ? 'Pantalla digital en escaparate — Apple Watch' : 'Digital signage in storefront — Apple Watch', width: 360, height: 480 },
                      ]}
                      caption={lang === 'ar' ? 'Digital signage — imágenes generadas con IA desde el catálogo de Airtable' : 'Digital signage — AI-generated images from the Airtable catalog'}
                    />
        ...
        <DataTable
          ...
          headers={[
            lang === 'ar' ? 'Módulo' : 'Module',
            lang === 'ar' ? 'Antes' : 'Before',
            lang === 'ar' ? 'Después' : 'After',
            lang === 'ar' ? 'Ahorro/mes' : 'Monthly Savings',
          ]}
          rows={[
        ...
        <Prose editorId="before-label" className="text-sm font-medium">
          {lang === 'ar' ? 'Así se gestionaba antes:' : 'How things were managed before:'}
        </Prose>
        <Photo1 editorId="before-backlog" src="/business-os/before-terminals-backlog.webp" alt={lang === 'ar' ? 'Terminales acumulados pendientes de recoger' : 'Devices piling up waiting for pickup'} width={1200} height={900} caption={lang === 'ar' ? 'Terminales acumulados pendientes de recoger' : 'Devices piling up waiting for pickup'} />
        <Photo2
          editorId="before-notes-pos"
          items={[
            { src: '/business-os/before-icloud-notes.webp', alt: lang === 'ar' ? 'Notas de iCloud como sistema de pedidos' : 'iCloud Notes as order system', width: 800, height: 1422 },
            { src: '/business-os/before-checkout-pos-stock.webp', alt: lang === 'ar' ? 'Checkout POS Manager — inventario manual' : 'Checkout POS Manager — manual inventory', width: 1200, height: 2133 },
          ]}
          caption={lang === 'ar' ? 'Pedidos en Notas de iCloud / Stock en Checkout POS' : 'Orders in iCloud Notes / Stock in Checkout POS'}
        />
        <Photo1 editorId="before-chaos" src="/business-os/before-workshop-chaos.webp" alt={lang === 'ar' ? 'Taller saturado' : 'Overwhelmed workshop'} width={1200} height={522} caption={lang === 'ar' ? 'Taller saturado — sin sistema de prioridades' : 'Overwhelmed workshop — no priority system'} />
        <Photo2
          editorId="before-sketch-founder"
          items={[
            { src: '/business-os/before-notebook-sketch.webp', alt: lang === 'ar' ? 'Boceto en libreta' : 'Notebook sketch', width: 800, height: 1067 },
            { src: '/business-os/before-founder-overwhelmed.webp', alt: lang === 'ar' ? 'Santiago en el taller' : 'Santiago in the workshop', width: 600, height: 800 },
          ]}
          caption={lang === 'ar' ? 'Primer boceto de integración / De este caos nació un Product Builder' : 'First integration sketch / This chaos built a Product Builder'}
          className="mb-6"
        />
        ...
        <Prose editorId="after-label" className="text-sm font-medium">
          {lang === 'ar' ? 'Después — todo en un solo sistema:' : 'After — everything in one system:'}
        </Prose>
        <Photo2
          editorId="after-store-orders"
          items={[
            { src: '/business-os/after-store-interior.webp', alt: lang === 'ar' ? 'Interior de Santifer iRepair' : 'Santifer iRepair interior', width: 1200, height: 900 },
            { src: '/business-os/after-orders-organized.webp', alt: lang === 'ar' ? 'Órdenes de trabajo organizadas' : 'Organized work orders', width: 1200, height: 900 },
          ]}
          caption={lang === 'ar' ? 'Un iMac, un sistema / Envíos COVID organizados con Airtable' : 'One iMac, one system / COVID shipments organized with Airtable'}
        />
        <Photo2
          editorId="after-workshop-accessories"
          items={[
            { src: '/business-os/after-workshop-organized.webp', alt: lang === 'ar' ? 'Taller organizado' : 'Organized workshop', width: 1200, height: 900 },
            { src: '/business-os/after-accessories-display.webp', alt: lang === 'ar' ? 'Estantería LED con accesorios' : 'LED shelf with accessories', width: 1024, height: 768 },
          ]}
          caption={lang === 'ar' ? 'Taller organizado / Cada accesorio con ubicación y stock en Airtable' : 'Organized workshop / Every accessory with location and stock in Airtable'}
          className="mb-8"
        />
        ...
        <CaseStudyCta
          ...
          heading={lang === 'ar' ? 'El mismo enfoque de sistemas, aplicado a LLMOps' : 'The same systems thinking, applied to LLMOps'}
          body={lang === 'ar'
          ...
          ctaLabel={lang === 'ar' ? 'Ver El Chatbot Que Se Cura Solo →' : 'Read The Self-Healing Chatbot →'}
          ctaHref={lang === 'ar' ? '/chatbot-que-se-cura-solo' : '/self-healing-chatbot'}
        />
